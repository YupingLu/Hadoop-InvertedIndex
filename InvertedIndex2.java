...
public class InvertedIndex2 extends Configured implements Tool {
  ...
  public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text> {
    ...
    public void configure(JobConf job) {
      caseSensitive = job.getBoolean("invertedindex2.case.sensitive", true);
      inputFile = job.get("map.input.file");
     
      filename = new Text(new File(inputFile).getName());

      if (job.getBoolean("invertedindex2.skip.patterns", false)) {
    ...
    public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
      ...
      //summary.set(filename.toString() + "|" + lines[0].replaceAll(" ", "") + "|"+ position++);
      //summary.set(filename.toString());
      ...
      while (tokenizer.hasMoreTokens()) {
        ...
        for (String retval: temp.split(" ")){
          if(retval != " " && !retval.isEmpty()){
            output.collect(new Text(retval), summary);
          }
        }
  ...
  public int run(String[] args) throws Exception {
    JobConf conf = new JobConf(getConf(), InvertedIndex2.class);
    conf.setJobName("invertedindex2");
    ...
    for (int i=0; i < args.length; ++i) {
      if ("-skip".equals(args[i])) {
        ...
        conf.setBoolean("invertedindex2.skip.patterns", true);
      } else {
